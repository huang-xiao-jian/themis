...
import type { IPaginatedDynamicResource, Pagination } from '../types/Resource';
import type { PaginatedFetcher, FieldDataSource } from '../types/Fetcher';

/**
 * 分页动态资源实现
 */
export class PaginatedResource<T extends FieldDataSource = FieldDataSource>
  implements IPaginatedDynamicResource<T>
{
  readonly name: string;
  readonly loading: IPaginatedDynamicResource<T>['loading'];
  readonly options: IPaginatedDynamicResource<T>['options'];
  readonly pagination: IPaginatedDynamicResource<T>['pagination'];
  ...
  private readonly _fetcher: PaginatedFetcher<T>;
  ...
  private async _fetch(): Promise<void> {
    this._loading(true);
    try {
      const result = await this._fetcher.fetch(this._pagination().page, this._pagination().pageSize);
